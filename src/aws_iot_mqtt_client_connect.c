...
static IoT_Error_t _aws_iot_mqtt_internal_connect(AWS_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	...
	if(NULL != pConnectParams) {
        ...
        IOT_INFO("pClient->networkStack.connect(&(pClient->networkStack), NULL)\n");
	rc = pClient->networkStack.connect(&(pClient->networkStack), NULL);
        IOT_INFO("After network stack connect\n");
	if(SUCCESS != rc) {
                IOT_INFO("Got an error during connect\n");
		/* TLS Connect failed, return error */
		FUNC_EXIT_RC(rc);
	}
        IOT_INFO("Get a timer connected\n");
	...
	pClient->clientData.keepAliveInterval = pClient->clientData.options.keepAliveIntervalInSec;
        IOT_INFO("Doing serialize connect()\n");
...
IoT_Error_t aws_iot_mqtt_connect(AWS_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	...
	FUNC_ENTRY;
        ...
        IOT_INFO("entering connect function\n");
	if(NULL == pClient) {
        IOT_INFO("entering connect function\n");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}

        IOT_INFO("asking aws_iot_mqtt_get_client_state\n");
	clientState = aws_iot_mqtt_get_client_state(pClient);
        IOT_INFO("received state, checking if state is valid for connect\n");

	if(false == _aws_iot_mqtt_is_client_state_valid_for_connect(clientState)) {
                IOT_INFO("network already connected error\n");
        ...
        IOT_INFO("looks like we are in valid state for connect. setting state to connecting\n");
	aws_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTING);
        IOT_INFO("set state to connected. doing _aws_iot_mqtt_internal_connect()\n");

	rc = _aws_iot_mqtt_internal_connect(pClient, pConnectParams);
        IOT_INFO("after aws_iot_mqtt_internal_connect, checking if it was successful\n");

	if(SUCCESS != rc) {
                IOT_INFO("Unsuccessful internal connect, disconnecting and returning error\n");
		...
		aws_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_DISCONNECTED_ERROR);
                ...
                IOT_INFO("successful client connect, setting state to connected\n");
		aws_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_CONNECTED_IDLE);
	}
        IOT_INFO("exiting aws_iot_mqtt_connect function\n");
